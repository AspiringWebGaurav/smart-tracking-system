...
export default function EnhancedVisitorStatusWatcher() {
  ...
  const initializeWatcher = async () => {
    try {
      // Get visitor UUID
      uuid.current = getOrCreateVisitorUUID();
  ...
  const handleBanAction = (status: VisitorStatus) => {
    ...
    showBanToast(() => {
      ...
      setTimeout(() => {
        ...
        window.location.href = `/ban?uuid=${uuid.current}&reason=${encodeURIComponent(status.banReason || 'Policy violation')}`;
      }, 1500);
    });
  };
