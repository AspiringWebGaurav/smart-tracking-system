...
export default function AdminDashboard() {
  ...
  const startRealTimeListeners = () => {
    ...
    try {
      console.log("🔥 Starting real-time Firebase listeners for admin dashboard");
      ...
      const appealsUnsubscribe = onSnapshot(
        ...
        (snapshot) => {
          ...
          const newAppeals = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
          })) as BanAppeal[];
          
          const newPendingCount = newAppeals.filter(appeal => appeal.status === 'pending').length;
          const currentPendingCount = appealStats.pending;
          
          // Show notification for new appeals
          if (newPendingCount > currentPendingCount) {
            showSuccessToast(`New ban appeal received! (${newPendingCount} pending)`);
          }
            ...
            <div className="flex items-center space-x-4">
              ...
              {isRealTimeActive && (
                <div className="flex items-center space-x-2 bg-green-500/20 border border-green-500/30 rounded-lg px-3 py-1">
                  ...
                  <span className="text-green-300 text-xs font-medium">Live Sync</span>
                </div>
              )}
